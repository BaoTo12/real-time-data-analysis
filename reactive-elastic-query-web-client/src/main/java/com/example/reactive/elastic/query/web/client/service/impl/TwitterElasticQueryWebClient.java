package com.example.reactive.elastic.query.web.client.service.impl;
...
import com.example.reactive.elastic.query.web.client.service.ElasticQueryWebClient;
    ...
    private WebClient.ResponseSpec getWebClient(ElasticQueryWebClientRequestModel requestModel) {
        return webClient
                ...
                .onStatus(
                        ...
                        cr -> {
                            HttpStatusCode status = cr.statusCode();
                            String reason = (status instanceof HttpStatus)
                                    ? ((HttpStatus) status).getReasonPhrase()
                                    : "Client error";
                            return Mono.error(new ElasticQueryWebClientException(reason));
                        }
                )
                .onStatus(
                        ...
                        cr -> {
                            ...
                            String reason = (status instanceof HttpStatus)
                                    ? ((HttpStatus) status).getReasonPhrase()
                                    : "Server error";
                            return Mono.error(new Exception(reason));
                        }
                );
    }
